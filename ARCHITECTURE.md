...
**Branding assets**: full-color brand marks live under `resources/branding/{stable,nightly}/` (`icon.icns` for the macOS bundle, `icon.ico` for Windows, `icon-1024.png` as the master, `icon-512.png` for Linux, `mark-256.png` for in-app use) plus the shared `resources/branding/glyph.svg`. `crates/dbflux_ui/src/assets.rs` serves the pre-rendered PNG mark per channel for `img(...)`; `glyph.svg` is monochrome and drawn with `currentColor` because `AppIcon::DbFlux` tints it like any other UI icon. The per-size Linux PNGs are committed under `packaging/icons/<size>x<size>/apps/`. The artwork is a raster master, so packaging copies these files rather than rasterising a vector at build time. Packaging metadata (`packaging/*.yaml`, `resources/desktop/dbflux.desktop`, `resources/macos/Info.plist`, `resources/windows/installer.iss`) and the Nix build (`nix/binary.nix`, `nix/nightly-info.nix`, `nix/release-info.nix`) substitute channel placeholders so the desktop entry, MIME association, and launcher icon match the running channel.
